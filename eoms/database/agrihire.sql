...
CREATE TABLE IF NOT EXISTS `agrihire`.`cart_item` (
  ...
  INDEX `shoppingcart_id` (`cart_id` ASC) VISIBLE,
  INDEX `shoppingcart_details_ibfk_2_idx` (`prodouct_code` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `agrihire`.`hire_record` (
  ...
  INDEX `booking_details_ibfk_10_idx` (`booking_item_id` ASC) VISIBLE,
  INDEX `booking_details_ibfk_19_idx` (`checkout_staff` ASC) VISIBLE,
  INDEX `booking_details_ibfk_29_idx` (`return_staff` ASC) VISIBLE,
...
-- -----------------------------------------------------
-- Table `agrihire`.`promo_store`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `agrihire`.`promo_store` (
  `promo_code` VARCHAR(10) NOT NULL,
  `store_id` INT NOT NULL,
  PRIMARY KEY (`promo_code`, `store_id`),
  INDEX `promotion_id` (`promo_code` ASC) VISIBLE,
  INDEX `promotion_record_ibfk_21_idx` (`store_id` ASC) VISIBLE,
  CONSTRAINT `prom_store_ibfk_1`
    FOREIGN KEY (`promo_code`)
    REFERENCES `agrihire`.`promotion` (`promo_code`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `promo_store_ibfk_2`
    FOREIGN KEY (`store_id`)
    REFERENCES `agrihire`.`store` (`store_id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
